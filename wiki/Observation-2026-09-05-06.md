...
- éviter toute capture astrométrique pendant un mouvement réel de la monture ;
...
L'étape « Position initiale » disparaît du workflow utilisateur.
...
- première fenêtre rapide : environ 0,90–1,51 arcsec/pixel ;
- hint AD/DEC INDI utilisé lorsqu'il est disponible ;
- fallback blind obligatoire avec la même fenêtre ;
- dernier fallback blind élargi : 0,50–2,50 arcsec/pixel.

Le solveur ne dépend jamais exclusivement de la qualité de la position annoncée par la monture.

## Interface tablette retenue

L'aperçu principal de la caméra est conçu pour une tablette utilisée verticalement :

- utilisation de toute la largeur disponible ;
- conservation du ratio du capteur Uranus-C ;
...
## Priorités restantes après intégration du 6 septembre

### P0

- valider sur CI puis sur le kit de distribution la suppression de l'orientation initiale ;
- valider le hint AD/DEC INDI et les fallbacks blind sur le Pi ;
- vérifier la motorisation OnStep en conditions réelles ;
- éviter une capture destinée au solve pendant un mouvement réel confirmé.

### P1

- améliorer la classification des captures manifestement non exploitables ;
- poursuivre la journalisation systématique des tentatives ;
- finaliser la mise au point Bahtinov guidée et son ergonomie terrain.

### P2

- renforcer les darks et les écrans de diagnostic après validation du cœur moteur/astrométrie/Bahtinov.
